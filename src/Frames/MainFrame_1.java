...
//        loadSelectedCourse();
    ...
    private boolean validateBatchForm(String batchNo, String course, String startDate, String tuitFee, String classAlloc) {
        if (batchNo.isEmpty() || course == null || course.isEmpty() || course.equals("Select") 
                || startDate.isEmpty() || tuitFee.isEmpty() || classAlloc.isEmpty()) {
            JOptionPane.showMessageDialog(this,
                    "All fields must be filled and a course must be selected.",
                    "Validation Error",
        ...
        try {
            int offNo = Integer.parseInt(batchNo);
            if (offNo <= 0) {
                JOptionPane.showMessageDialog(this,
                        "Batch No (Offering No) must be a positive integer.",
                        "Validation Error",
                        JOptionPane.ERROR_MESSAGE);
                return false;
            }
            ...
            JOptionPane.showMessageDialog(this,
                    "Batch No (Offering No) must be a valid number.",
                    "Validation Error",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        }
        ...
        try {
            java.time.LocalDate.parse(startDate, java.time.format.DateTimeFormatter.ISO_LOCAL_DATE);
        } catch (java.time.format.DateTimeParseException e) {
            JOptionPane.showMessageDialog(this,
                    "Start Date must be a valid calendar date in YYYY-MM-DD format (e.g., 2026-06-15).",
                    "Validation Error",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        }
        ...
        try {
            double fee = Double.parseDouble(tuitFee);
            if (fee <= 0) {
                JOptionPane.showMessageDialog(this,
            ...
            JOptionPane.showMessageDialog(this,
                    "Tuition Fee must be a valid numeric value.",
                    "Validation Error",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        }
        ...
        try {
            int room = Integer.parseInt(classAlloc);
            if (room <= 0) {
                JOptionPane.showMessageDialog(this,
                        "Classroom Allocation (Room No) must be a positive integer.",
                        "Validation Error",
                        JOptionPane.ERROR_MESSAGE);
                return false;
            }
            ...
            JOptionPane.showMessageDialog(this,
                    "Classroom Allocation (Room No) must be a valid number.",
                    "Validation Error",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        }
    ...
    public void insertData() {
        ...
        if (!validateBatchForm(batchNo, course, startDate, tuitFee, classAlloc)) {
            return;
        }

        String query = "INSERT INTO course_offering (offering_no, start_date, tuition_fee, course_code, room_no) VALUES (?,?,?,?,?)";
    ...
    private void setupComboBoxListener() {
        cbxSelectCourse.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                String query = "SELECT course_fee FROM course WHERE course_code = ?";
                try {
    ...
    private void updateBatch() {
        ...
        if (tblAddBatch.getSelectedRowCount() == 1) {
            ...
            if (!validateBatchForm(batchNo, courseCode, stDate, tuitFee, clsAlloc)) {
                return;
            }

            try {
                String query = "UPDATE course_offering SET offering_no=?, start_date=?, tuition_fee=?, course_code=?, room_no=? WHERE course_offering_id=?";
    ...
    private void deleteStudent() {
        ...
        if (tblAddBatch.getSelectedRowCount() == 1) {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                ...
                try {
                    String query = "DELETE FROM course_offering WHERE course_offering_id = ?";
